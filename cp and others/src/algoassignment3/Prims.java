...
public class Prims {
	...
	public static void main(String[] args) {
		...
		System.out.println("Enter the start vertex for Prim's algorithm");
		int start = s.nextInt();
		dijkstra(g, g.nodes[start]);

	}

	private static void dijkstra(graph g, nodes node) { // change name of this function to prim's
		...
		while (!isEmpty(minHeap)) {
			...
			for (int i = 0; i < minHeapNode.node.adj.size(); i++) {
				...
				if (isInMinHeap(minHeap, v) && dist[u] != Integer.MAX_VALUE
						&& minHeapNode.node.adj.get(i).wt < dist[v]) {
					dist[v] = minHeapNode.node.adj.get(i).wt;
					g.nodes[v].parent = g.nodes[u];
		...
		for (int i = 0; i < dist.length; i++) {
			if(i==src) continue;
			System.out.println(g.nodes[i].parent.index+"--"+i);
		}
